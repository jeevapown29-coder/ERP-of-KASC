...
export default function Attendance() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ==========================================================================
         BIOMETRIC SCANNING STATION & HUD SIMULATOR
         ========================================================================== */}
      <div className="bg-slate-900 border-2 border-emerald-500/30 rounded-2xl p-6 shadow-[0_0_20px_rgba(16,185,129,0.1)] relative overflow-hidden">
        {/* Futuristic Grid Background pattern */}
        <div className="absolute inset-0 bg-[radial-gradient(#10b981_1px,transparent_1px)] [background-size:16px_16px] opacity-10 pointer-events-none" />
        
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-6">
          
          {/* Column 1: Scanner Controller Interface */}
          <div className="lg:col-span-1 space-y-4">
            ...
            <h2 className="text-xl font-extrabold text-white tracking-tight flex items-center gap-2">
              ...
              Biometric Scan Station
            </h2>
            
            <p className="text-[11px] text-slate-400 leading-relaxed font-mono">
              Interface securely with local high-resolution hardware scanners for instant registrar enrollment confirmation.
            </p>

            {/* Selector for Sensor Subsystem */}
            <div className="grid grid-cols-2 gap-2 bg-slate-950 p-1 rounded-xl border border-slate-800">
              <button 
                onClick={() => { setBioMethod('FINGER'); if (bioStatus === 'AUTHENTICATED') setBioStatus('CONNECTED'); }}
                className={`py-2 text-[10px] font-extrabold uppercase rounded-lg transition-all flex items-center justify-center gap-1 font-mono ${bioMethod === 'FINGER' ? 'bg-emerald-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'}`}
              >
                <Fingerprint className="w-3.5 h-3.5" /> Fingerprint
              </button>
              <button 
                onClick={() => { setBioMethod('FACE'); if (bioStatus === 'AUTHENTICATED') setBioStatus('CONNECTED'); }}
                className={`py-2 text-[10px] font-extrabold uppercase rounded-lg transition-all flex items-center justify-center gap-1 font-mono ${bioMethod === 'FACE' ? 'bg-emerald-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'}`}
              >
                <Video className="w-3.5 h-3.5" /> Facial Recognition
              </button>
            </div>

            {/* Scanner Device Signal Hud */}
            <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 space-y-2 text-xs font-mono">
              <div className="flex items-center justify-between">
                ...
                <span className="text-slate-300">KASC-BIO-SCANNER-V4</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">Link Status:</span>
                <span className="text-emerald-400 font-bold flex items-center gap-1">
                  <Wifi className="w-3.5 h-3.5" /> SECURE ONLINE
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500">Encryption:</span>
                <span className="text-slate-300">SHA-256 LEDGER LINK</span>
              </div>
            </div>
          </div>

          {/* Column 2: Interactive Cyber Scanning Frame */}
          <div className="lg:col-span-1 flex flex-col items-center justify-center relative">
            
            {/* The Scanner Touch Plate Area */}
            <div 
              onClick={triggerBiometricSimulationScan}
              className={`w-40 h-40 rounded-2xl border-2 cursor-pointer transition-all duration-300 relative flex flex-col items-center justify-center overflow-hidden group select-none ${
                bioStatus === 'SCANNING' ? 'border-amber-400 bg-amber-500/5 shadow-[0_0_25px_rgba(245,158,11,0.2)]' :
                bioStatus === 'ANALYZING' ? 'border-sky-400 bg-sky-500/5 shadow-[0_0_25px_rgba(56,189,248,0.2)] animate-pulse' :
                bioStatus === 'AUTHENTICATED' ? 'border-emerald-500 bg-emerald-500/10 shadow-[0_0_35px_rgba(16,185,129,0.25)] scale-102' :
                'border-slate-700 bg-slate-950 hover:border-emerald-400/50 hover:shadow-[0_0_15px_rgba(16,185,129,0.1)]'
              }`}
            >
              
              {/* Dynamic Scan Ray Beam (moving line animation) */}
              {(bioStatus === 'SCANNING' || bioStatus === 'ANALYZING') && (
                <div className={`absolute left-0 right-0 h-1 z-20 shadow-[0_0_10px_currentColor] animate-[bounce_2s_infinite] ${
                  bioStatus === 'SCANNING' ? 'bg-amber-400 text-amber-500' : 'bg-sky-400 text-sky-500'
                }`} />
              )}

              {/* Graphic inside plate */}
              {bioMethod === 'FINGER' ? (
                <Fingerprint className={`w-16 h-16 transition-transform duration-300 group-hover:scale-105 ${
                  bioStatus === 'SCANNING' ? 'text-amber-400' :
                  bioStatus === 'ANALYZING' ? 'text-sky-400' :
                  bioStatus === 'AUTHENTICATED' ? 'text-emerald-400 animate-bounce' :
                  'text-slate-600 group-hover:text-emerald-400'
                }`} />
              ) : (
                <div className="relative">
                  <Video className={`w-16 h-16 transition-transform duration-300 group-hover:scale-105 ${
                    bioStatus === 'SCANNING' ? 'text-amber-400' :
                    bioStatus === 'ANALYZING' ? 'text-sky-400' :
                    bioStatus === 'AUTHENTICATED' ? 'text-emerald-400 animate-bounce' :
                    'text-slate-600 group-hover:text-emerald-400'
                  }`} />
                  {/* Cyber viewfinder brackets */}
              ...
              {/* Action caption text underneath */}
              <span className="text-[9px] font-mono font-black tracking-widest uppercase mt-4 text-slate-500 group-hover:text-emerald-400">
                {bioStatus === 'SCANNING' ? 'READING SENSOR...' :
                 bioStatus === 'ANALYZING' ? 'MATCHING DB...' :
                 bioStatus === 'AUTHENTICATED' ? 'VERIFIED MATCH' :
                 'TAP TO LOG ATTENDANCE'}
              </span>
            </div>

            {/* Verification Success / Error Panel feedback */}
            <div className="mt-3 text-center h-6">
              {bioStatus === 'SCANNING' && <p className="text-[10px] font-mono text-amber-400 font-bold tracking-widest animate-pulse">⚡ HIGH-RESOLUTION PATTERN SCAN TRIGGERED</p>}
              {bioStatus === 'ANALYZING' && <p className="text-[10px] font-mono text-sky-400 font-bold tracking-widest animate-pulse">🛠️ DECRYPTING BIOMETRIC TOKEN ARRAYS...</p>}
              {bioStatus === 'AUTHENTICATED' && activeBioUser && (
                <p className="text-[11px] font-mono text-emerald-400 font-black tracking-wider uppercase">
                  ✔️ MATCH CONFIRMED {activeBioUser.accuracy}% ACCURACY
                </p>
              )}
              {bioStatus === 'CONNECTED' && <p className="text-[10px] text-slate-500 font-mono tracking-wider">TOUCH PAD OR USE CAMERA TO SIMULATE LOGIN</p>}
            </div>
          </div>

          {/* Column 3: Live Verification Ledger Log & Results */}
          <div className="lg:col-span-1 space-y-3 flex flex-col justify-between">
            {activeBioUser && bioStatus === 'AUTHENTICATED' ? (
              /* Verified Profile Result Card */
              <div className="p-4 bg-slate-950 rounded-xl border border-emerald-500/40 shadow-[0_0_15px_rgba(16,185,129,0.15)] flex flex-col justify-center space-y-3 animate-in fade-in zoom-in-95 duration-200 h-full">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-emerald-500/10 border border-emerald-500/40 flex items-center justify-center text-lg select-none">
                    🔑
                  </div>
                  <div>
                    <h4 className="text-sm font-black text-white leading-tight">{activeBioUser.name}</h4>
                    <span className="text-[10px] uppercase font-mono font-bold tracking-wider text-emerald-400">{activeBioUser.role}</span>
                  </div>
                </div>

                <div className="space-y-1.5 text-[10px] font-mono text-slate-400 bg-slate-900 p-2.5 rounded-lg border border-slate-800">
                  <div className="flex justify-between"><span className="text-slate-500">REGISTRAR ID:</span> <span>{activeBioUser.reg}</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">COHORT BATCH:</span> <span className="truncate max-w-[130px]">{activeBioUser.group}</span></div>
                  <div className="flex justify-between"><span className="text-slate-500">COMPLIANT OK:</span> <span className="text-emerald-400 font-bold">YES - GRANTED</span></div>
                </div>
              </div>
            ) : (
              /* Historical Database Logging Feed */
              <div className="space-y-2 h-full flex flex-col justify-between">
                <h4 className="text-[10px] font-black tracking-widest text-[#f09a1a]/90 uppercase font-mono border-b border-slate-800 pb-1.5 flex items-center justify-between">
                  <span>🛰️ Live Scanner Ledger</span>
                  <span className="text-[9px] px-1.5 py-0.5 bg-emerald-500/10 text-emerald-400 rounded-md">Ledger active</span>
                </h4>
                
                <div className="space-y-1.5 flex-1 overflow-y-auto pr-1 max-h-36">
                  {bioLog.map((logItem) => (
                    <div key={logItem.id} className="p-2 bg-slate-950 rounded-lg border border-slate-850 flex items-center justify-between text-[10px] font-mono gap-2">
                      <div className="truncate">
                        <p className="font-bold text-white truncate">{logItem.name}</p>
                        <p className="text-[8px] text-slate-500">{logItem.method} • {logItem.time}</p>
                      </div>
